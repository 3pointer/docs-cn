...
TiKV 节点收到备份请求后，会遍历自己的全部 Region Leader，找到和请求中 KV Range 有重叠的范围，将该范围下的部分或者全部 Region Leader 进行备份，这样在备份路径下生成对应的 SST 文件（命名格式是 storeID_regionID_regionEpoch_tableID)。
